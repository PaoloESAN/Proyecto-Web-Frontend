...
const emit = defineEmits<{
  (e: "submit", payload: { dniFrontal: File; dniPosterior: File }): void;
}>();
...
const canSubmit = computed(() => {
  return dniFrontalFile.value !== null && dniPosteriorFile.value !== null && !props.uploading;
});

function handleSubmit() {
  if (!canSubmit.value) return;
  emit("submit", {
    dniFrontal: dniFrontalFile.value!,
    dniPosterior: dniPosteriorFile.value!,
  });
}
...
<template>
  <div class="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6 sm:p-8 shadow-sm space-y-8">
    ...
    <div class="space-y-3">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <UButton icon="i-lucide-trash" color="error" variant="ghost" size="xs" class="cursor-pointer" @click="removeFrontal" />
        </div>
    ...
    <div class="space-y-3">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <UButton icon="i-lucide-trash" color="error" variant="ghost" size="xs" class="cursor-pointer" @click="removePosterior" />
        </div>
        <div class="aspect-video w-full max-h-48 rounded-lg overflow-hidden border border-neutral-100 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-950 flex items-center justify-center">
    ...
    <UButton
      ...
      color="primary"
      size="lg"
      class="w-full font-bold justify-center py-3 cursor-pointer"

  ...
  <div
    ...
    <UCard
      ...
      <UAlert v-if="errorMessage" color="error" variant="soft" icon="i-lucide-alert-circle"
        :title="errorMessage ? '' : 'Error al registrarse'" :description="errorMessage" class="mb-6" />

      <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
        <div class="flex flex-col sm:flex-row gap-4">
          <UFormField name="nombres" label="Nombres" required class="flex-1">
            <UInput v-model="state.nombres" type="text" placeholder="Juan" icon="i-lucide-user" class="w-full"
              autocomplete="given-name" />
          </UFormField>

          <UFormField name="apellidos" label="Apellidos" required class="flex-1">
            <UInput v-model="state.apellidos" type="text" placeholder="Pérez" icon="i-lucide-user" class="w-full"
              autocomplete="family-name" />
          </UFormField>
        </div>

        <UFormField name="correo" label="Correo electrónico" required>
          <UInput v-model="state.correo" type="email" placeholder="correo@ejemplo.com" icon="i-lucide-mail"
            class="w-full" autocomplete="email" />
        </UFormField>

        <UFormField name="password" label="Contraseña" required>
          <UInput v-model="state.password" type="password" placeholder="Mínimo 6 caracteres" icon="i-lucide-key-round"
            class="w-full" autocomplete="new-password" />
        </UFormField>

        <UFormField name="confirmarPassword" label="Confirmar contraseña" required>
          <UInput v-model="state.confirmarPassword" type="password" placeholder="••••••••" icon="i-lucide-key-round"
            class="w-full" autocomplete="new-password" />
        </UFormField>

        <div class="pt-2">
          <UButton type="submit" label="Registrarse" color="primary" block
            class="font-semibold py-2.5 transition-transform active:scale-[0.98]" :loading="loading" />
        </div>
      </UForm>
